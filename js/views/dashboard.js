import { listStagiaires, listEvaluations, getStats, getSetting } from "../db.js?v=20260703b";
import { el, clear, isoDate, getMonday, displayStagiaire, compareByNom } from "../utils.js?v=20260703b";
import { icon } from "../icons.js?v=20260703b";

const SORT_OPTIONS = [
  { key: "priorite",   label: "PrioritÃ© de passage" },
...
// prio = "tours utilisÃ©s" = passages faits (+ bonus) + refus/absences. Un refus compte comme un
// tour : il fait perdre la place de prioritaire, mais le vrai compteur (effectif) reste plus bas.
// ComptÃ©s UNIQUEMENT depuis les passages enregistrÃ©s (table `passages`, via Â« Valider la
// semaine Â» ou saisie manuelle) â€” le planning seul ne compte pas â†’ suivi sans ambiguÃ¯tÃ©.
function statForType(s, type) {
  const e = s?.[type]?.["EffectuÃ©"] || 0;
...
function labelText(key) {
  return { "a-prioriser": "Ã€ prioriser", "a-jour": "Ã€ jour" }[key];
}
...
function badgeTitle(stat, objectif) {
  ...
  return `ComptÃ© ${stat.prio} (${detail}) Â· objectif ${objectif}`;
}

/** Moyenne pondÃ©rÃ©e /20 des Ã©valuations d'un stagiaire. null si pas d'Ã©valuations notÃ©es. */
...
function buildStatPill(iconFn, stat) {
  ...
  if (stat.bonus > 0) pill.appendChild(el("span", { class: "add" }, "+" + stat.bonus + "â˜…"));
  if (stat.absence > 0) pill.appendChild(el("span", { class: "miss" }, stat.absence + "âœ•"));
  return pill;
}
...
function priorityBadge(scope, key, retard, stat, objectif) {
  return el("span", { class: "priority-badge " + key, title: badgeTitle(stat, objectif) },
    ...
    retard > 0 ? el("span", { class: "retard", title: "retard sur la moyenne" }, "âˆ’" + retard) : null,
  );
}
...
function renderCard(x, objSalle, objVoiture) {
  ...
  return el("article", { class: "dashboard-card " + cardClass(prioSalle, prioVoiture) },
    el("div", { class: "card-head" },
      ...
      avg != null
        ? el("span", { class: "avg-pill " + avgColor(avg), title: nbEvals + " Ã©valuation(s)" },
            el("span", { class: "avg-num" }, String(avg)),
            el("span", { class: "avg-max" }, "/20"),
          )
        : el("span", { class: "avg-pill muted", title: "Aucune Ã©valuation" },
            el("span", { class: "avg-num" }, "â€”"),
          ),
    ),
...
function explainPanel(objSalle, objVoiture) {
  return el("details", { class: "dash-explain" },
    el("summary", {}, "Comment la prioritÃ© est calculÃ©e ?"),
    el("div", { class: "dash-explain-body" },
      el("p", {},
        el("strong", {}, "Les passages se comptent Ã  Â« Valider la semaine Â»."),
        " Placer quelqu'un dans le planning, c'est planifier â€” Ã§a ne compte pas tant que la semaine n'est pas validÃ©e. Le tableau de bord n'affiche que les passages enregistrÃ©s."),
      el("p", {},
        el("strong", {}, "Objectif = moyenne de la classe"),
        ` (cette semaine : Salle ${objSalle}, Voiture ${objVoiture}). Au niveau ou au-dessus â†’ `,
        el("em", {}, "Ã  jour"), ". En dessous â†’ ", el("em", {}, "Ã  prioriser"),
        " ; plus le retard est grand, plus c'est prioritaire."),
      el("p", {},
        el("strong", {}, "Un refus ou une absence compte comme un tour utilisÃ©."),
        " Il fait perdre sa place de prioritaire â€” refuser n'est donc pas Â« gratuit Â». Mais le vrai compteur de passages, lui, reste en dessous : on redevient prioritaire dÃ¨s que la classe avance. ",
        el("strong", {}, "On rattrape son retard, le refus a juste coÃ»tÃ© un tour.")),
      el("p", { class: "muted" },
        "Sur les compteurs : âœ• = refus / absence (comptÃ©s dans la prioritÃ©) Â· â˜… = passage bonus."),
    ),
  );
}
...
export async function renderDashboard(container) {
  ...
  // Objectif par type = moyenne de classe du compteur de prioritÃ© (faits + bonus + refus), arrondie.
  ...
  const enriched = stagiaires.map((s) => {
    ...
    const score = retardSalle + retardVoiture;  // tri prioritÃ© : plus de retard d'abord
  ...
  container.appendChild(el("div", { class: "view-header" },
    el("div", { class: "view-header-text" },
      ...
      el("p", { class: "subtitle" }, "Qui doit passer en prioritÃ©, par rapport Ã  la moyenne de la classe."),
    ),
  ));
  ...
  container.appendChild(el("div", { class: "dashboard-meta" },
    el("div", { class: "stat-block" },
      ...
      el("span", { class: "label" }, "Ã€ prioriser"),
    ),
    el("div", { class: "stat-block" },
      el("span", { class: "value" }, String(summary.aJour)),
      el("span", { class: "label" }, "Ã€ jour"),
    ),
    el("div", { class: "stat-block" },
      el("span", { class: "value" }, objSalle + " Â· " + objVoiture),
      el("span", { class: "label" }, "Objectif S Â· V"),
    ),
    el("div", { class: "stat-block" },
  ...
  container.appendChild(el("div", { class: "dashboard-legend" },
    el("span", {}, el("span", { class: "dot", style: "background:var(--c-stop)" }), "Ã€ prioriser : sous la moyenne (le nombre = retard)"),
    el("span", {}, el("span", { class: "dot", style: "background:var(--c-go)" }), "Ã€ jour : au niveau du groupe (â‰¥ moyenne)"),
    el("span", {}, "âœ• = refus/absence (comptÃ© comme un tour) Â· â˜… = bonus"),
  ));
}
