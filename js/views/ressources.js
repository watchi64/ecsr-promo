...
import { isAdmin } from "../auth-admin.js?v=20260703b";
import { recordUndo } from "../undo.js?v=20260703b";

let ressources = [];
let contacts = [];

const CATEGORIES_ORDER = ["Officiel", "RÃ©fÃ©rentiels", "PÃ©dagogie", "Examens", "Autre"];
...
function openEditModal(existing, onSaved) {
  ...
  const urlInput = el("input", { type: "url", placeholder: "https://â€¦", value: existing?.url || "" });
  ...
  async function save() {
    ...
    try {
      ...
      toast(isNew ? "Ressource ajoutÃ©e" : "Ressource mise Ã  jour", "success");
      backdrop.remove();
      onSaved();
    } catch (e) {
      console.error(e);
  ...
  const modal = el("div", { class: "modal" },
    ...
    el("div", { class: "modal-form" },
      ...
      el("div", { class: "field" }, el("label", {}, "CatÃ©gorie"), catSel),
...
function renderRessourceCard(r, onChange) {
  ...
  if (admin) {
    ...
    const delBtn = el("button", {
      ...
      onClick: async (ev) => {
        ev.preventDefault();
        if (!confirm(`Supprimer Â« ${r.titre} Â» ?`)) return;
...
function openContactModal(existing, onSaved) {
  ...
  const prenomInput = el("input", { type: "text", placeholder: "PrÃ©nom (ou nom)", value: existing?.prenom || "" });
  const roleInput = el("input", { type: "text", placeholder: "RÃ´le / fonction (ex. Service entreprise â€” CP)", value: existing?.role || "" });
  ...
  async function save() {
    if (!prenomInput.value.trim()) { toast("PrÃ©nom requis", "error"); return; }
    const payload = {
    ...
    try {
      if (isNew) {
        ...
        toast("Contact ajoutÃ© Â· Ctrl+Z pour annuler", "success", 2400);
        if (inserted?.id) recordUndo("contact ajoutÃ©", async () => { await deleteContact(inserted.id); });
      } else {
        const prev = { ...existing };
        await updateContact(existing.id, payload);
        recordUndo("contact modifiÃ©", async () => {
          await updateContact(existing.id, {
            prenom: prev.prenom, role: prev.role, phone: prev.phone,
            email: prev.email, note: prev.note, category: prev.category,
          });
        });
        toast("Contact mis Ã  jour", "success", 1800);
      }
      backdrop.remove();
      onSaved();
  ...
  const modal = el("div", { class: "modal" },
    ...
    el("div", { class: "modal-form" },
      el("div", { class: "field" }, el("label", {}, "PrÃ©nom"), prenomInput),
      el("div", { class: "field" }, el("label", {}, "RÃ´le"), roleInput),
      el("div", { class: "field-pair" },
        el("div", { class: "field" }, el("label", {}, "TÃ©lÃ©phone"), phoneInput),
...
function renderContactCard(c, onChanged) {
  ...
  if (c.phone) {
    ...
    links.appendChild(el("a", { class: "contact-link phone", href: "tel:" + tel },
      el("span", { class: "contact-link-icon" }, "ðŸ“ž"),
      el("span", {}, c.phone),
    ));
  }
  if (c.email) {
    links.appendChild(el("a", { class: "contact-link email", href: "mailto:" + c.email },
      el("span", { class: "contact-link-icon" }, "âœ‰ï¸"),
      el("span", {}, c.email),
    ));
  }
  ...
  if (admin) {
    ...
    const delBtn = el("button", {
      ...
      onClick: async () => {
        ...
        recordUndo("contact supprimÃ©", async () => { await addContact(snapshot); });
        onChanged();
      }
    });
    delBtn.appendChild(icon.trash());
...
function rerender(container) {
  ...
  container.appendChild(el("div", { class: "view-header" },
    el("div", { class: "view-header-text" },
      el("p", { class: "eyebrow" }, ressources.length + " liens curÃ©s Â· " + contacts.length + " contacts"),
      el("h2", {}, "Ressources & contacts"),
      el("p", { class: "subtitle" }, "Contacts administration (absences, justificatifs) + textes officiels & sites pÃ©dagogiques."),
    ),
  ));
  ...
  contactsSection.appendChild(el("div", { class: "contacts-section-head" },
    el("h3", { class: "ressource-section-title" }, "ðŸ“ž Contacts administration"),
    admin ? el("button", {
  ...
  contactsSection.appendChild(el("p", { class: "muted contacts-section-sub" },
    "NumÃ©ros Ã  appeler en cas d'absence ou de question administrative. Tape sur un numÃ©ro pour appeler directement."));

  if (contacts.length === 0) {
    contactsSection.appendChild(el("p", { class: "muted", style: "padding:1rem;text-align:center" }, "Aucun contact enregistrÃ©."));
  } else {
  ...
  // En-tÃªte de la zone ressources (avec bouton "Ajouter" Ã  droite, admin only)
  container.appendChild(el("div", { class: "contacts-section-head", style: "margin-top:1.5rem" },
    el("h3", { class: "ressource-section-title", style: "margin:0" }, "ðŸ“š Ressources externes"),
    admin ? el("button", {
      class: "btn small accent",
  ...
  container.appendChild(el("p", { class: "muted contacts-section-sub", style: "margin-bottom:1rem" },
    "Liens vers les textes officiels, rÃ©fÃ©rentiels, sites pÃ©dagogiques."));
